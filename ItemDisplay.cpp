...
string ItemDescLookupCache::make_cached_T(UnitItemInfo* uInfo)
{
	...
	for (vector<Rule*>::const_iterator it = RuleList.begin(); it != RuleList.end(); it++)
	{
		...
		{
			// Only update description if the rule specifies one (non-empty)
			string appliedDesc = (*it)->ApplyDescription(ctx);
			if (!appliedDesc.empty()) {
				ctx.name = appliedDesc;
			}
	...
	const string& name)
{
	...
	for (vector<Rule*>::const_iterator it = RuleList.begin(); it != RuleList.end(); it++)
	{
		if ((*it)->Evaluate(uInfo))
		{
			// Only update name if the rule specifies a name (non-empty action.name)
			// Rules with only map/notify directives should not clear the item name
			string appliedName = (*it)->ApplyName(ctx);
			if (!appliedName.empty()) {
				ctx.name = appliedName;
			}
			if ((*it)->action.stopProcessing) { break; }
		}
	}
